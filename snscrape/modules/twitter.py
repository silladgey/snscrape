...
class _TwitterAPIScraper(snscrape.base.Scraper):
    ...
    def _make_card(self, card, apiType, tweetId):
        ...
        if cardName in (
            ...
            kwargs = snscrape.utils.dict_map(
                bindingValues,
                {
            ...
            return SpacesCard(
                **snscrape.utils.dict_map(
                    bindingValues, {"card_url": "url", "id": "id"}
                )
            )
    ...
    def _make_edit_state(self, editControl):
        ...
        return EditState(
            ...
            editableUntilDate=datetime.datetime.fromtimestamp(
                int(editControl["editable_until_msecs"]) / 1000,
    ...
    def _graphql_timeline_instructions_to_tweets(
        self, instructions, includeConversationThreads=False, **kwargs
    ):
        for instruction in instructions:
            ...
            for entry in instruction["entries"]:
                if entry["entryId"].startswith("tweet-"):
                    ...
                    ("homeConversation-", "profile-conversation-")
                ):
                    if entry["content"]["entryType"] == "TimelineTimelineModule":
                        for item in reversed(entry["content"]["items"]):
                            if (
                                not item["entryId"].startswith(
                                    entry["entryId"].split("ion-", 1)[0] + "ion-"
                                )
                    ...
                    for item in entry["content"]["items"]:
                        if item["entryId"].startswith(f'{entry["entryId"]}-tweet-'):
                            if item["entryId"][len(entry["entryId"]) + 7 :].strip(
                                "0123456789"
                            ):
                                _logger.warning(
    ...
    def _render_text_with_urls(self, text, urls):
        ...
        for url, nextUrl in itertools.zip_longest(urls, urls[1:]):
            ...
            out.append(
                text[
                    url["indices"][1] : (
                        nextUrl["indices"][0] if nextUrl is not None else None
                    )
                ]
            )
        return "".join(out)
    ...
    def _user_to_user(self, user, id_=None, **kwargs):
        ...
        if "label" not in kwargs and (
            labelO := user.get("affiliates_highlighted_label", {}).get("label")
        ):
...
class TwitterSearchScraper(_TwitterAPIScraper):
    ...
    def get_items(self):
        ...
        paginationVariables = {
            ...
            "cursor": None,
            "product": (
                "Latest" if self._mode is TwitterSearchScraperMode.LIVE else "Top"
            ),
...
class TwitterTweetScraper(_TwitterAPIScraper):
    ...
    def get_items(self):
        ...
        if self._mode is TwitterTweetScraperMode.SINGLE:
            ...
            for obj in self._iter_api_data(
                ...
                hasModeratedReplies = (
                    hasModeratedReplies
                    or self._has_moderated_replies(obj, self._tweetId)
                )
            ...
            while queue:
                ...
                for obj in self._iter_api_data(
                    ...
                    hasModeratedReplies = (
                        hasModeratedReplies or self._has_moderated_replies(obj, tweetId)
                    )
                if hasModeratedReplies:
    ...
    def _get_moderated_replies(self, tweetId):
        ...
        paginationParams = {"variables": paginationVariables, "features": features}
        url = (
            "https://twitter.com/i/api/graphql/pOVQRe-x12WZeawviP7zxw/ModeratedTimeline"
        )
        instructionsPath = [
            "data",
            "tweet",
        ...
        for obj in self._iter_api_data(
            ...
            yield from self._graphql_timeline_instructions_to_tweets(
                obj["data"]["tweet"]["result"]["timeline_response"]["timeline"][
                    "instructions"
                ],
                includeConversationThreads=True,
            )
...
class TwitterUsersScraper(_TwitterAPIScraper):
    ...
    def get_items(self):
        ...
        obj = self._get_api_data(
            "https://twitter.com/i/api/graphql/GD4q8bBE2i6cqWw2iT74Gg/UsersByRestIds",
            _TwitterAPIType.GRAPHQL,
            params={"variables": variables, "features": features},
